...
import com.renen.fjgr.bean.BookInfo;
import com.renen.fjgr.bean.CourseInfo;
import com.renen.fjgr.bean.GradeExam;
...
public class JsoupService {
	...
	public static final String paramEmptyClassroom[] ={"kssj","jssj","xqj","ddlDsz","sjd"};
    /**
     * 判断是否登录成功
     *
	...
	public static  List<Object> parseEvaluateParam(String content){
		List<Object> FormDatas=new ArrayList<Object>();
		
		Document document = Jsoup.parse(content);
		Elements hiddens = document.select("input[type=hidden]");
		Elements optionsDoc = document.select("option[selected=selected]");
		Elements suggestion = document.select("textarea[name=pjxx]");
		Elements SubmitButton=document.select("input[name=Button2]");
		
		Map<String,String> hidensMap = new HashMap<String, String>();
		List<String> optionsList=new ArrayList<String>();
		//Map<String,String> selectedMap = new HashMap<String, String>();
		
		for (Element en : hiddens){
			hidensMap.put(en.attr("name"),en.attr("value"));
		}
		FormDatas.add(hidensMap);
		//注
		for (Element en : optionsDoc){
			//.put(en.attr("name"),en.attr("value"));
			optionsList.add(en.attr("value"));
		}
		FormDatas.add(optionsList);

        FormDatas.add(URLDecoder.decode( suggestion.text() ));
		FormDatas.add((SubmitButton.get(0)).attr("disabled"));
		
		return FormDatas;
	}
	
	
	public static Map<String,List<String>> getCourseTableParamVal(String content){
		final String param[] ={"xn","xq","nj","xy","zy","kb"};
		Map<String, List<String>> map = new HashMap<>();
        Document document = Jsoup.parse(content);
        Elements elements = document.select("table[id=Table1]").select("tbody");
		
		for(String p : param){
			Elements options=elements.select("select[name=" +p +"] option");
			List<String> optionsList=new ArrayList<String>();
			for(Element en : options){
				optionsList.add(en.val());
			}
			map.put(p,optionsList);
			//optionsList.clear();
		}
		return map;
	}
	...
	public static Map<String,List<String>> getCourseTableParam(String content){
		final String param[] ={"xn","xq","nj","xy","zy","kb"};
		Map<String, List<String>> map = new HashMap<>();
        Document document = Jsoup.parse(content);
        Elements elements = document.select("table[id=Table1]").select("tbody");
		//Log.e("zz",elements.toString());
		int i=0;
		
		List<Map<String,String>> paramval=new ArrayList<Map<String,String>>();
		for(String p : param){
			Elements options=elements.select("select[name=" +p +"] option");
			List<String> optionsList=new ArrayList<String>();
			for(Element en : options){
				optionsList.add(en.text());
			}
		...
		for (Element en : hiddens){
			hiddenList.add(en.val());
			
		}
		
		map.put("baseParam",hiddenList);
		
		return map;
	}
	
	public static Map<String,String> getCurrentCourseTableParam(String content){
		final String param[] ={"xn","xq","nj","xy","zy","kb"};
		Map<String, String> map = new HashMap<>();
        Document document = Jsoup.parse(content);
        Elements elements = document.select("table[id=Table1]").select("tbody");
		...
		for(String p : param){
			Elements options=elements.select("select[name=" +p +"] option");
			for(Element en : options){
				if(en.attr("selected").equals("selected"))
					map.put(p,en.text());
			}
		}
	...
	public static Map<String,List<String>> getEmptyClassroomParam(String content){
		//final String param[] ={"kssj","jssj","xqj","ddlDsz","sjd"};
		Map<String, List<String>> map = new HashMap<>();
        Document document = Jsoup.parse(content);
       // Elements elements = document.select("table[id=Table1]");
		...
		List<Map<String,String>> paramval=new ArrayList<Map<String,String>>();
		for(String p : paramEmptyClassroom){
			//Elements options=elements.select("select[name=" +p +"] option");
			Element options=document.getElementById(p);
			List<String> optionsList=new ArrayList<String>();
			for(Element en : options.getElementsByTag("option")){
				optionsList.add(en.text());
			}
    ...
    public static String[][] getCourse(String content) {
        ...
        elements.remove(4);
        elements.remove(5);
        String[][] course = new String[5][7];
		Log.e("zzz",elements.toString());
        for (int i = 0; i < elements.size(); i++) {
            ...
            for (int j = 0; j < elements1.size(); j++) {
				...
				String text=elements1.get(j).html().replace("<br>", "\n").replace("&nbsp;", "");
                course[i][j] = text;
            }
        }
        return course;
    ...
    public static Map<String, Object> getScoreYear(String content) {
       ...
       // map.put("__VIEWSTATEGENERATOR", elements.get(0).val());
		...
		elements = document.select("form[name=Form1]");
		map.put("SubmitLink", URLDecoder.decode( elements.get(0).attr("action")));
    ...
    public static ArrayList<CourseInfo> parseCourseScore(String content) {
        ...
        Elements elements = document.select("table[id=Datagrid1]").select("tbody").select("tr");
        //移除无效项
        elements.remove(0);
