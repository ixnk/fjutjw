...
import com.renen.fjgr.cookie.CookiesManager;
...
public class OkHttpUtil {
     ...
     * Host
     */
    private static final String HOST = "jw.fjuts.com:8000";
    /**
     * Referer
     */
    private static final String REFERER = "http://jw.fjuts.com:8000/";
    /**
     * 验证码请求地址
     */
    private static final String URL_CODE = "http://jw.fjuts.com:8000/CheckCode.aspx";
    /**
     * 登录地址
     */
    private static final String URL_LOGIN = "http://jw.fjuts.com:8000/default2.aspx";
	...
	public static Request getRequest(String url,Map<String,String> headerMap,RequestBody requestBody ){
		Request.Builder requestBiilder = new Request.Builder().url(url).addHeader("Host", HOST).addHeader("Referer", url).post(requestBody);
		
		for (Map.Entry entry : headerMap.entrySet()) {
			//System.out.println("key= " + entry.getKey() + " and value= " + entry.getValue());
			requestBiilder.addHeader((String)entry.getKey(),(String)entry.getValue());
		}
		return requestBiilder.build();
	}
	...
	public static String getWebContent(Request request,Context context,Handler handler){
		
		OkCallback callback= new OkCallback(context,handler);
		
		
		OkHttpUtil.getOkHttpClient().newCall(request).enqueue(callback);
		
		return callback.getWebContent();
